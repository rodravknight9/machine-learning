...
|-----------|--------------|---------|
| **Drop empty rows** | Remove rows where all cells are missing. | `dropna(how='all')`. See [ventas.ipynb](ventas.ipynb). |
| **Filter by string condition** | Remove bad rows (e.g. repeated header lines) using a logical condition on a column. | e.g. keep only rows where the first characters of a date column are not `'Or'`. See [ventas.ipynb](ventas.ipynb). |
| **Imputation (mean)** | Fill missing numeric values with the column mean. | `SimpleImputer(strategy='mean')` fit on non-missing data, then transform. See [exportdata.ipynb](exportdata.ipynb) with [Data.csv](Data.csv). |
...
- **Multi-file concatenation:** Read several CSV files (e.g. monthly sales), then merge with `pd.concat` into one DataFrame. Optionally export to a single CSV. **Example:** [ventas.ipynb](ventas.ipynb).
- **Single CSV load and slice:** Load with `pd.read_csv`, then use `iloc` to separate features (e.g. `iloc[:, :-1]`) and target (e.g. last column). **Example:** [exportdata.ipynb](exportdata.ipynb).
...
- **Label encoding:** Map categories to integers with `LabelEncoder`. Good for the **target** (e.g. Yes/No → 0/1). For **features**, it can imply a false order (e.g. Germany > France). **Example:** [exportdata.ipynb](exportdata.ipynb).
- **One-hot encoding:** Replace one categorical column with dummy (0/1) columns, one per category. Use `OneHotEncoder(sparse_output=False)` and combine with numeric columns (e.g. `np.column_stack`). **Example:** [exportdata.ipynb](exportdata.ipynb).

### 3.4 Train/test splitting

- **Random split (sklearn):** `train_test_split(df, test_size=0.2, random_state=42)` for a reproducible holdout. **Example:** [exportdata.ipynb](exportdata.ipynb), [example.ipynb](../textbooks/hands-on-machine-learning/example.ipynb).
...
- **Standardization (z-score):** `StandardScaler`—fit on train, transform train and test; features get mean 0, std 1. **Example:** [exportdata.ipynb](exportdata.ipynb).
- **Normalization (min-max):** Scale to a fixed range (e.g. [0, 1]). Formula is often mentioned in preprocessing references; [exportdata.ipynb](exportdata.ipynb) uses StandardScaler in code.
...
- **Bar chart (by category):** `groupby` + `sum()` or `count()`, then `plt.bar(categories, values)`—e.g. sales by month, sales by city. Use `.index` and `.values` from the groupby result. **Example:** [ventas.ipynb](ventas.ipynb).
- **Line chart (time/sequence):** `groupby` (e.g. by hour) + `count()` or `sum()`, then `plt.plot(positions, values)` for orders by hour. **Example:** [ventas.ipynb](ventas.ipynb).
- **Products bought together:** Concatenate product list per order with `groupby('Order ID')['Product'].transform(...)`, drop duplicates, then count pairs (e.g. with `itertools.combinations` and `Counter`). **Example:** [ventas.ipynb](ventas.ipynb).
...
1. **Suggested order:** Read the foundational concepts (Section 1), then open the notebooks and follow along:
   - **[exportdata.ipynb](exportdata.ipynb)** — full preprocessing pipeline: load CSV, impute, label and one-hot encode, train/test split, StandardScaler.
   ...
   - **[ventas.ipynb](ventas.ipynb)** — sales EDA: consolidate monthly CSVs, clean, feature engineering (month, city, sales), bar and line charts, products bought together.
